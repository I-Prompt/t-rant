...
"Angry message → professional rewrite" already exists as a category (Angry Email Translator, Anger Translator, AI Corporate Translator, among others). Two things set this one apart:

- **A real guardrail architecture.** A dedicated, isolated safety classifier runs *before* anything gets rewritten — not three prompt variations bolted onto an unguarded text box.
- **Provable transparency, not "trust us."** Every block shows you exactly what tripped it, in your own words. The privacy claims link straight to the logging code so you can check them yourself.

Everything below is built and working today, including the guardrail pipeline, the pixel-art identity, the self-harm calming redesign, and full multilingual support across 7 languages. The only thing still outstanding is a human native-speaker review pass on the non-English safety translations — see [Status](#status).
...
- 📤 [Sharing](#sharing)
- 🎨 [Visual & Sound Identity](#visual--sound-identity)
- ✨ [Extras](#extras)
- 📊 [Status](#status)
...
Code-generated pixel art and a warm, muted color palette — no external image assets anywhere in the app (this README's banner above follows the same rule: it's a hand-authored SVG built from the exact same sprite geometry as the in-app mascot, not a screenshot).

- **Pixel T-Rex sprite** ([`lib/rexSprite.ts`](app/src/lib/rexSprite.ts)): one shared 18×14 silhouette with a distinct pose per tone/pathway — raised eyebrow, necktie, olive branch, a stomping loading animation, a stop sign + meteor for witty blocks. Deliberately **no sprite at all** for Hard NO or the serious pathway — those stay unbranded on purpose.
...
- **Pixel favicon** ([`icon.tsx`](app/src/app/icon.tsx)): server-rendered from the same sprite geometry, so it can never drift from the in-app mascot.
...
- **[Unwind links](app/src/lib/unwindLinks.ts)**: Tetris, explore.org, r/aww, The Useless Web, 2048, Wordle — shown after any non-serious response, and reachable directly at [`/unwind`](app/src/app/unwind/page.tsx).
...
Full build history lives in commit messages; this is the narrative version, newest first.
...
Next.js 16 (App Router, TypeScript) plus the Anthropic SDK, both pipeline stages on Claude Haiku. Deploys to Vercel. No accounts, no database: classification category and a timestamp are the only things ever logged, and raw rant text is never persisted.
