...
**2026-08-21 pass: modes, guardrails, and a copy/data audit.** Two sitewide modes via a shared context (`UIState.tsx`): **dark mode** (full dark token palette, toggled or system-detected, persisted) and **stealth mode** (one click disguises the page as a plausible "Notes" app - decoy to-do notes fill the textarea itself, even before you've typed anything, with a non-interactive project-tracker panel beside it and a tiny "for display only" disclaimer; deliberately *never* persisted, since a saved flag would undercut the point). A third, **reader mode** (`?reader=1`), strips the page to just the textarea and the result. The result view shows Professional & Clear immediately, with the other two tones and Director's Cut behind an expander, personas behind "More tones," and unwind links/privacy behind "Learn more." Sharing was redesigned twice: an X-share flow was built, then replaced with a tick-box picker on "Copy shareable link" - check any combination of the three tones and five personas, and only what's checked goes into the link. Also this pass: two free anti-abuse guardrails on every API route (an origin/referer check rejecting requests that didn't come from the site itself, and a minimum-interval throttle blunting scripted bursts within the hourly cap - see [`requestGuard.ts`](app/src/lib/requestGuard.ts) and [`rateLimit.ts`](app/src/lib/rateLimit.ts)), a cross-reference audit fixing several stale pointers between House Rules, Dark Pattern Audit, Bookmarklet, and Emergency Numbers, a large expansion of the emergency-numbers list (see Extras below), and a real bug fix: the bookmarklet's `javascript:` link was silently neutered by a newer React version's href-sanitizing security feature, fixed by setting the attribute imperatively outside JSX instead of through a prop. None of this touches the self-harm/in-danger pathway, which stays exactly as unbranded as before.
...
- **Persistent left-nav sidebar** (all pages): Unwind Links, House Rules, and Dark Pattern Audit get the top slots, then a divider, then Emergency Numbers and the Bookmarklet - each with its own emoji, current page highlighted. Also carries the dark-mode toggle, and (home page only) a dismissible "rant haiku of the day."
...
- **Shareable link, à la carte**: a checkbox picker lets you choose any combination of the three tones and five personas; only what's ticked gets encoded into the URL - never your original draft, never anything you didn't select. No backend storage. Personas are generated on demand and cached, so ticking the same one for the link and for the "More tones" preview only costs one API call.
...
- **[Emergency numbers reference](app/src/app/emergency-numbers/page.tsx)**: a region-then-country dropdown showing a general emergency phone number, available standalone and embedded in the serious pathway - as the lead resource for in_danger (immediate physical danger comes first in that flow, see Guardrail categories above), as a secondary reference alongside `findahelpline.com` for self_harm. Most countries also show smaller-font secondary numbers (non-emergency police, domestic-violence hotlines, poison control, etc.) where there was reasonable confidence in a real, stable number - these double as the practical answer for "hurt by someone, no immediate danger" in the in_danger flow, once a country is picked. 60 entries total, each cross-checked against Wikipedia's "List of emergency telephone numbers" plus targeted searches, in two passes: 2026-08-17 (35 entries; one real correction came out of it, India's ambulance figure 102 → 108) and 2026-08-21 (25 more, mainly to round out a thin Middle East & Africa list, 4 countries → 23). Every entry shows its own "last verified" date in the UI, and a few 2026-08-21 entries show a visible note flagging a genuine source conflict (e.g. Lebanon's fire number) rather than silently picking one. Deliberately excluded: Palestine/West Bank/Gaza (a product decision, not an oversight) and Papua New Guinea (researched, but no single reliable national number could be confirmed - see the file's maintenance note for both). Built instead of IP-based geolocation, which was explicitly ruled out. See the maintenance note in [`emergencyNumbers.ts`](app/src/lib/emergencyNumbers.ts) for how to re-verify going forward.
...
**2026-08-21 pass:** dark mode with a full token palette and persisted/system-detected theme; stealth mode (disguises the page as a plausible "Notes" app - decoy to-do notes fill the textarea by default, a non-interactive project-tracker panel sits beside it, both with a "Rules" disclosure hiding a "for display only" disclaimer - session-only, never persisted); reader mode (`?reader=1`, bare textarea + result, no chrome); sidebar reordered (Unwind Links, House Rules, Dark Pattern Audit, then a divider, then Emergency Numbers and the Bookmarklet) with an emoji on every link and the old House-Rules highlight background removed; a shorter default result view (Professional & Clear shown immediately, the other two tones/Director's Cut behind an expander, personas behind "More tones," unwind links/privacy footer behind "Learn more"); the writing-guidance callout collapses by default and can be manually re-collapsed; auto-growing textarea; auto-scroll to the result; a dismissible "rant haiku of the day" in the sidebar (home page only); a confetti burst, randomized branded-card caption, copy-stamp animation, page-load curtain transition, and sticky "back to top" mini-header past the fold; four new sound effects (hero-click roar, clean-result success chime, loading-message tick, plus the existing tone blips); Wordle added to the unwind links; tagline changed to "They probably deserved it." A tick-box picker on "Copy shareable link" replaced an earlier X-share flow, letting you choose any combination of tones and personas for the link. Two free anti-abuse guardrails added to every API route. A cross-reference audit fixed several stale pointers (Dark Pattern Audit → House Rules, Bookmarklet's privacy claim, House Rules' own self-references) and a factual error (the help-now buttons are below the textarea, not above). The emergency-numbers list grew from 35 to 60 entries, mainly filling out a thin Middle East & Africa section. A real bug was found and fixed: the bookmarklet stopped working after a React version bump started sanitizing `javascript:` hrefs set via JSX - fixed by setting the attribute imperatively instead. Also fixed: the sidebar's sticky positioning was silently broken sitewide by `overflow-x: hidden` on `body` (switched to `overflow-x: clip`), and the ads/paywall House Rules answer never actually said "no" before listing what it meant. One more small fix after first shipping this pass: the persona buttons ("Nature Documentary" was the one that showed it) could wrap their label onto a second line on a narrower viewport - fixed with `white-space: nowrap` plus a slightly smaller font on both the "More tones" buttons and the shareable-link checkboxes. None of this touches the self-harm/in-danger pathway.
